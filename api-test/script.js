...
const loading = document.querySelector('#iLoading')

window.addEventListener('DOMContentLoaded', buscarPost = async () => {
    const responseUser = await fetch(`https://jsonplaceholder.typicode.com/posts`)
    const users = await responseUser.json() 
    const response = await fetch(`https://jsonplaceholder.typicode.com/posts`)
    const posts = await response.json() 
    await posts.sort((a, b) => b.id - a.id).map((item) => {
        // div .post
        ...
        const abrirComentar = async (event) => {
            ...
            const responsePost = await fetch(`https://jsonplaceholder.typicode.com/comments?postId=${item.id}`)
            const coment = await responsePost.json()
